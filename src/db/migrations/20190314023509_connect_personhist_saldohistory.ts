exports.up = async (knex: any) => {
    ...
    const connectedSaldoIds = new Set(itemhistory.map((itemEvent: any) => itemEvent.saldhistid));
    ...
    const unconnectedSaldoEvents = saldohistory.filter((saldoEvent: any) => !connectedSaldoIds.has(saldoEvent.saldhistid));
    ...
    await knex.schema.table('PERSONHIST', (table: any) => {
