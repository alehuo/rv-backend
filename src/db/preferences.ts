...
export const getPreference = async (preference: any) => {
...
export const setPreference = async (preference: any, value: any) => {
    const data = await knex('PREFERENCES').where({ key: preference.key }).first('value');
...
const floatPreference = {
    ...
    deserialize: (str: any) => parseFloat(str),
    validate: (value: any) => (typeof value === 'number' ? [] : ['value should be a number'])
};

const integerPreference = {
    serialize: (value: any) => String(value),
    deserialize: (str: any) => parseInt(str),
    validate: (value: any) =>
        typeof value !== 'number'
...
export const GLOBAL_DEFAULT_MARGIN = preferences.GLOBAL_DEFAULT_MARGIN;
export const DEFAULT_PRODUCT_CATEGORY = preferences.DEFAULT_PRODUCT_CATEGORY;

export const getPreferenceByKey = (key: string) => {
